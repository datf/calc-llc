	...
	function buyQty() {
		if (canAfford) {
			...
			gameState.inventory[selectedItem.itemID] =
				(gameState.inventory[selectedItem.itemID] || 0n) + inputQty;
			showPopup = false;
		}
	}
...
{#if showPopup && selectedItem}
	<div
		class="fixed inset-0 z-50 flex items-center justify-center var(--bg-main)/50"
		onmousedown={(e) => {
			if (e.target === e.currentTarget) showPopup = false;
		}}
	>
		<div
			class="theme-surface theme-surface-hover p-6 rounded border theme-border shadow-2xl flex flex-col gap-4 w-80 theme-text"
		>
			<h3 class="text-xl theme-text-accent font-bold text-center border-b theme-border pb-2">
			...
			<div class="flex flex-col gap-2">
				...
				<input
					...
					class="theme-surface border theme-border rounded px-2 py-2 theme-text text-lg text-center font-bold"
				/>
			</div>
			<div class="flex flex-col gap-2 mt-2">
				<button
					onclick={buyQty}
					disabled={!canAfford}
					class="bg-green-700 hover:bg-green-600 disabled:bg-gray-700 disabled:theme-text-muted py-2 rounded font-bold"
				>
					Buy (Cost: ${formatLargeNumber(totalCost)})
				</button>
				<button
					onclick={sellQty}
					disabled={(gameState.inventory[selectedItem.itemID] || 0n) < inputQty}
					class="bg-red-700 hover:bg-red-600 disabled:bg-gray-700 disabled:theme-text-muted py-2 rounded font-bold"
				>
					Sell (Refund: ${formatLargeNumber(totalSell)})
				</button>
				<button
					onclick={setQty}
					class="bg-blue-700 hover:bg-blue-600 py-2 rounded font-bold mt-2 border-t theme-border pt-2"
				>
					Set Quantity
				</button>
			</div>
					...
					>
						...
						<div class="flex gap-3 flex-1 mb-4 w-full">
							...
							<div class="flex-1 text-xs text-gray-300 flex flex-col gap-1">
								{#if item.damage}<p>
										Dmg: <span class="theme-text font-bold">{formatLargeNumber(item.damage)}</span>
									</p>{/if}
								{#if item.Strength}<p>
										Str: <span class="theme-text">{formatLargeNumber(item.Strength)}</span>
									</p>{/if}
								{#if item.cooldown_secs}<p>
										CD: <span class="theme-text">{item.cooldown_secs}s</span>
									</p>{/if}
							</div>
						</div>
					</button>
						...
						<div class="grid grid-cols-4 gap-2">
							{#each activeInventory as { item, qty }}
								...
								>
									...
									<div
										class="absolute -bottom-2 -right-2 bg-gray-700 theme-text text-[10px] font-bold px-1 rounded border theme-border z-10 shadow"
									>

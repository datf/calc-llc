			...
			if (item.kind === 'file') {
				...
				if (item.getAsFileSystemHandle) {
					try {
						...
						console.warn('Failed to get handle from drop', err);
					}
				}
...
>
	{#if showReloadAnimation}
		<div
			class="absolute top-20 left-1/2 -translate-x-1/2 z-[200] bg-green-600 text-white px-6 py-3 rounded-full shadow-lg font-bold transition-opacity animate-pulse"
		>
			Save Auto-Reloaded!
		</div>
	{/if}
	...
	{#if isDragging}
		<div
			class="absolute inset-0 z-[100] bg-black/60 flex items-center justify-center backdrop-blur-sm pointer-events-none"
		>
			<div
				class="text-4xl font-bold text-white border-4 border-dashed border-white p-12 rounded-2xl pointer-events-none"
			>
				Drop Save File Here
			</div>
		</div>
	...
	<div class="shrink-0 shadow-2xl z-50">
		<header
			class="bg-transparent theme-text-accent p-4 border-b-2 theme-border flex justify-between text-sm sm:text-base"
		>
			<!-- Left Column -->
			<div class="flex flex-col gap-2">
				<div class="flex items-center gap-1">
					...
					{:else}
						<button
							...
							class="font-bold underline cursor-pointer hover:theme-text px-1"
						>
							{formattedCash}
						</button>
					{/if}
              ...
              {fileHasChanged
							...
							: 'theme-border theme-surface-hover theme-text-muted'}"
					...
					{#if fileHandle}
						<label
							class="flex items-center gap-1 text-sm theme-text-muted cursor-pointer hover:theme-text"
						>
			...
			<div class="flex flex-col gap-2 text-right">
				<div>
					...
					{:else}
						<button
							onclick={() => (isEditingProfession = true)}
							class="theme-text hover:underline cursor-pointer"
						>
							{GAME_OPTIONS.professions.find((p) => p.profession_id === gameState.professionId)
								?.profession_name || gameState.professionId}
						</button>
					{/if}
				</div>
	...
	<main
		class="flex-1 overflow-y-auto bg-[var(--bg-main)] text-[var(--text-main)] transition-colors duration-300"
	>

...
<div class="page-wrapper page-wide">
  <div class="mb-6 text-center">
    ...
    <p class="text-base theme-text-muted max-w-2xl mx-auto">
      Purchase systemic upgrades on the left, and adjust your global passive multipliers on the right.
    </p>
  </div>
  ...
  <div class="flex flex-col lg:flex-row gap-8">
    ...
    <div class="flex-1">
      <h2 class="text-2xl font-bold mb-4 theme-text-accent border-b theme-border pb-2">Bonus Upgrades</h2>

      {#if availableBonusItems.length === 0}
        <div class="theme-surface rounded border theme-border p-8 text-center">
          <p class="theme-text-muted italic">There are no more bonus items available in the shop.</p>
        </div>
      {:else}
            ...
            >
              ...
              <div class="flex gap-3 flex-1 mb-2 w-full">
                ...
                <div class="flex-1 text-xs text-gray-300 flex flex-col justify-center">
                  <span class="theme-text-muted italic mb-1 uppercase tracking-wider">{item.itemType}</span>
    ...
    <div class="lg:w-80 shrink-0">
      <div class="sticky top-4">
        <h2 class="text-2xl font-bold mb-4 theme-text-accent border-b theme-border pb-2">Passive Effects</h2>
        
        <!-- Added max height and vertical scrolling so it doesn't break the page height -->
        <div class="theme-surface p-4 rounded border theme-border max-h-[75vh] overflow-y-auto custom-scrollbar">
          <div class="flex flex-col gap-4 pr-1">
            {#each PASSIVE_KEYS as key}
              <div class="flex flex-col bg-black/20 p-3 rounded border theme-border">
                <label for={key} class="block text-[11px] uppercase tracking-wider font-bold theme-text-muted mb-2">
                  {formatLabel(key)}
                </label>

<script>
	...
	let heldItem = $derived(employee.equipment_itemID ? items.get(employee.equipment_itemID) : null);
	...
	let sellPrice = $derived(
		employee.itemSellPrice ? BigInt(employee.itemSellPrice) : employee.upgrade_cost / 2n
	);
...
{#if showPopup}
	...
	>
		<div
			class="bg-gray-800 p-6 rounded border border-gray-600 shadow-2xl flex flex-col gap-4 w-80 text-white"
		>
			<h3 class="text-xl text-coal-gold font-bold text-center border-b border-gray-700 pb-2">
			...
			<div class="flex flex-col gap-2">
				...
				<input
					...
					class="bg-gray-900 border border-gray-600 rounded px-2 py-2 text-white text-lg text-center font-bold"
				/>
			</div>
			...
			<div class="flex flex-col gap-2 mt-2">
				<button
					onclick={buyQty}
					disabled={!canAfford || !hasPrerequisiteForQty}
					class="bg-green-700 hover:bg-green-600 disabled:bg-gray-700 disabled:text-gray-500 py-2 rounded font-bold transition-colors"
				>
					Buy (Cost: ${formatLargeNumber(totalCost)})
				</button>
				<button
					onclick={sellQty}
					disabled={ownedCount < inputQty}
					class="bg-red-700 hover:bg-red-600 disabled:bg-gray-700 disabled:text-gray-500 py-2 rounded font-bold transition-colors"
				>
					Sell (Refund: ${formatLargeNumber(totalSell)})
				</button>
				<button
					onclick={setQty}
					class="bg-blue-700 hover:bg-blue-600 py-2 rounded font-bold transition-colors mt-2 border-t border-gray-600 pt-2"
				>
					Set Quantity (Free)
				</button>
			</div>
...
<div class="flex flex-col items-center">
			...
			{#each employee.promotions as promo, index}
				<div class="relative flex flex-col items-center px-4">
					...
					{#if employee.promotions.length > 1}
						<div
							class="absolute bottom-0 h-0.5 bg-red-700 {index === 0
								? 'left-1/2 right-0'
								: index === employee.promotions.length - 1
									? 'left-0 right-1/2'
									: 'left-0 right-0'}"
						></div>
					{/if}
	...
	<div class="relative z-10 flex flex-col items-center mb-2">
		{#if ownedCount > 0n}
			...
			<div
				class="absolute -top-3 -right-3 bg-green-500 text-white text-xs font-bold px-1 rounded border border-black z-20"
			>
		...
		<button
			class="relative w-16 h-20 bg-gray-400 border-2 border-gray-600 shadow-md cursor-pointer hover:border-white transition-all flex justify-center {!isEnabled
		...
		>
			<div style="image-rendering: pixelated;" class="relative w-10 h-full flex justify-center">
				{#if employee.wings_texture_base64}
					<img
						src={employee.wings_texture_base64}
						alt="Wings"
						class="absolute h-10 z-0 max-w-none left-1/2 -translate-x-1/2"
						style="width: 120px; top: 16px;"
					/>
				{/if}
				{#if employee.legs_texture_base64}
					<img
						src={employee.legs_texture_base64}
						alt="Legs"
						class="absolute w-10 h-10 z-10"
						style="top: 24px;"
					/>
				{/if}
				{#if employee.torso_texture_base64}
					<img
						src={employee.torso_texture_base64}
						alt="Torso"
						class="absolute w-10 h-10 z-20"
						style="top: 24px;"
					/>
				{/if}
				{#if employee.head_texture_base64}
					<img
						src={employee.head_texture_base64}
						alt="Head"
						class="absolute w-10 h-10 z-30"
						style="top: 8px;"
					/>
				{/if}
				...
				{#if heldItem && heldItem.picB64}
					<img
						src={heldItem.picB64}
						alt={heldItem.itemName}
						class="absolute w-8 h-8 z-40 transform rotate-45"
		...
		<div
			class="mt-1 bg-amber-100 text-black text-xs font-mono px-1 border border-dashed border-gray-600"
		>

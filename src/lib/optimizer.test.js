...
describe('Optimizer Integration (Real Items)', () => {
	...
	it('should not sell the silver shotgun if it is locked', () => {
		const realGameState = {
			cash: 19185n,
			inventory: {
				shotgun_06_silver: 1
		...
		const lockedItems = ['shotgun_06_silver'];
		const results = calculateBestUpgrades(
			realGameState,
			itemsMap,
			employeeMap,
			'MAX_DPS',
		...
		expect(soldItemNames).not.toContain('Silver Shotgun');

		// Prevented the Amethyst Shotgun purchase due to lack of funds
		const newShotgun = option1.buy.items.find((i) => i.name === 'Amethyst Shotgun');
		expect(newShotgun).toBeUndefined();
	});
